...
/***************************************************************************
*	Maintenance versions by Ron Fredericks at BiophysicsLab.com		       *
*																		   *
*	New version 0.5	RDF	of phpmathpublisher.php forked code, Nov 14, 2022  *
*		Fix > symbol issue with current WordPress Gutenberg shortcode 	   *
*			> alternate usage with gt									   *
*			>= alternate usage with ge									   *
*			< and <= include optional lt and le operators for symmetry     *
*			original > >= < <= symbols still supported where useful		   *
*		Add del and Del (aka Nabla) symbol for div grad curl calculus	   *
*	Version 0.4	RDF	a fork of phpmathpublisher.php code, Sept 6, 2019  	   *	
*		Fix two issues for use on PHP 7									   *
*			1) replace deprecated ereg() with preg_match()				   *
*			2) eliminate warning class constructor with  __construct 	   *
*																		   *
****************************************************************************/
...
'gamma'=>'&#176;',
'delta'=>'&#177;',
'del'=>'&#8711;',		// Add vector differential operator (div grad curl) RDF
'epsilon'=>'&#178;',
'varepsilon'=>'&#34;',
'zeta'=>'&#179;',
'eta'=>'&#180;',
...
'<'=>'&#60;',
'>'=>'&#62;',
'gt'=>'&#62;',  //Eliminate problem with > symbol, RDF
'lt'=>'&#60;',  //For consistancy add < symbol too, RDF
//parentheses
'('=>'&#179;',
')'=>'&#180;',
'['=>'&#104;',
...
'gamma'=>'cmmi10',
'delta'=>'cmmi10',
'del'=>'FreeSerif',		//Add vector differential operator (div grad curl) RDF
'epsilon'=>'cmmi10',
'varepsilon'=>'cmmi10',
'zeta'=>'cmmi10',
'eta'=>'cmmi10',
...
'le'=>'msam10',
'ge'=>'msam10',
'lt'=>'cmmi10',		//Add for > consistancy for < symbol, RDF
'gt'=>'cmmi10',		//Eliminate problem with > symbol, RDF
'<'=>'cmmi10',
'>'=>'cmmi10',
//parentheses
'('=>'cmex10',
...
{
//return ereg("^[0-9]", $str);  # ereg deprecated in php 7
...
for($i = 0; $i < count($t); $i++)
	...
	//if (ereg("formula", $t[$i]))  # ereg deprecated in php 7
...
$texte=stripslashes($texte);
if(isset($fontesmath[$texte])) $font = $dirfonts."/".$fontesmath[$texte].".ttf";
//elseif (ereg("[a-zA-Z]", $texte)) $font = $dirfonts."/cmmi10.ttf";  # ereg deprecated in php 7
...
{
// leave original function name constructor for backward compatibility with PHP 3 and 4
function expression_texte($exp)
{
$this->texte = $exp;
}
// use __construct(...) to avoid future deprecation warning in php 7
...
while ($fi = readdir($handle))
	{
	$info=pathinfo($fi);
	//if ($fi!="." && $fi!=".." && $info["extension"]=="png" && ereg("^math",$fi)) # ereg deprecated in php 7
